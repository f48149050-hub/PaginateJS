...
async function redisRequest(path: string, method = 'GET', value?: string) {
...
async function storeKey(apiKey: string, record: object): Promise<void> {
    await redisRequest(`/set/apikey:${apiKey}`, 'POST', JSON.stringify(record));
}

async function storeCustomerKey(customerId: string, apiKey: string): Promise<void> {
    await redisRequest(`/set/customer:${customerId}`, 'POST', JSON.stringify({ apiKey }));
}
...
async function getExistingKey(customerId: string): Promise<string | null> {
    ...
    return JSON.parse(result).apiKey ?? null;
}
...
async function sendKeyEmail(email: string, apiKey: string): Promise<void> {
    await fetch('https://api.resend.com/emails', {
        ...
        body: JSON.stringify({
            from: `PaginateJS <${process.env.YOUR_FROM_EMAIL ?? 'keys@paginatejs.com'}>`,
            to: email,
        ...
        <div style="font-family:sans-serif;max-width:500px;margin:0 auto;padding:32px">
          ...
          <div style="background:#f4f4f4;border-radius:8px;padding:16px;margin:24px 0;font-family:monospace;font-size:14px;word-break:break-all">${apiKey}</div>
